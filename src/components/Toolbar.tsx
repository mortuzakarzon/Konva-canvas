...
import { Square, Circle, MousePointer, Type, Pencil, Minus, Trash2, RotateCcw, RotateCw, Image, RefreshCw, Copy, Palette } from 'lucide-react';
...
const Toolbar: React.FC = () => {
  ...
  const [fillColor, setFillColor] = useState('#3886F6');
  const [strokeColor, setStrokeColor] = useState('#3886F6');
    ...
    if (selectedId) {
      ...
      if (shape) {
        ...
        if (shape.type === 'text') {
          ...
          setFillColor(shape.fill || '#3886F6');
          setStrokeColor(shape.stroke || '#3886F6');
          // Extract opacity from rgba fill color
          const match = shape.fill?.match(/[\d.]+\)$/);
          if (match) {
            setOpacity(parseFloat(match[0]));
          }
        }
      } else {
  ...
  const handleOpacityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedShape?.type === 'rectangle' || selectedShape?.type === 'ellipse') {
      updateShape({
        ...selectedShape,
        fill: `rgba(${hexToRgb(fillColor)}, ${newOpacity})`
      });
    }
  };
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  updateShape({
                    ...selectedShape,
                    fill: `rgba(${hexToRgb(e.target.value)}, ${opacity})`
                  });
                }}
                className="h-8 w-8 p-0 border border-gray-200 rounded cursor-pointer"
                title="Fill Color"
